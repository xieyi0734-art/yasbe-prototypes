...
### 1.1 背景
YASBe 通过 OpenPayd 为用户提供一条**欧元 / 英镑法币账户通道**：平台为已完成建档的主体（个人 / 企业）开立 OpenPayd 法币账户（Account），账户 ACTIVE 后系统获得该账户专属的**收款账户信息（Payment Account）**；用户从**本人（同名）的外部银行账户**向收款账户转账完成入金，平台凭 OpenPayd 的 Pay In 到账通知在自有账本为用户入账；出金时用户只能选择**已添加、同名且状态正常（已核验）的外部账户（EA）**作为收款目的地，平台调用 OpenPayd 发款。

> 一句话：**法币账户是"入金收款管道"，外部同名账户是"出金收款目的地"，平台余额是"用户可支配记账"；同名（户主=本人/本企业）是出入金强约束——同名自动放行（入账 / 创建 / 出金），非同名须上传证明 + 运营审核、且仅登记为入金来源（2026-09-08）。**
>
...
- G2 合规对齐：主体建档先行（个人 / 企业主体）；同名强约束产品化——入金到账自动判定同名（同名自动入账 / 非同名挂账闭环）、出金目的地仅限同名已核验 EA（添加同名自动成功 / 非同名证明 + 运营审核且仅作入金来源）。
...
| 外部账户 / EA | External Account（EA） | 用户添加的外部银行账户，按户名是否与主体（个人全名 / 企业注册名）**同名**分流：同名 → 自动创建成功（可出金）；非同名 → 上传证明 + 运营审核（通过仅登记为入金来源，不可出金）。OpenPayd 侧对应 **Beneficiary（收款人档案）+ Bank Beneficiary（银行收款账户）** 两层对象 |
...
| 同名判定 / 持有人审核 | Same-name check / CoP | 平台对账户持有人与主体实名是否**同名**的自动判定（大小写不敏感 + 忽略空格，F2）：入金 senderName 与 EA 持有人输入两侧统一；同名 → 自动放行（入账 / EA 创建）；非同名 → 证明 + 运营审核（EA 添加，D7）/ 入金挂账（D8）。英国同名 EA 可选叠加 CoP（`POST /beneficiaries/verify`，MATCH 等）作真实性增强（❓ F4）；欧洲无 CoP 端点（F3） |
...
- 入金到账与上账：Pay In 到账通知 → 平台同名判定：同名 → 自动入账；**非同名 → 挂账闭环（提示添加该外部账户并上传证明 → 运营审核放行 / 驳回退回；7 天未添加自动退款、可手动撤销；退款执行机制 ❓ §12 F1）**。
- 外部账户（EA）同名分流：添加（= 新建 Beneficiaries + bank Beneficiaries 两层）、持有人可输入 + 实时同名判定——同名自动创建成功 / 非同名上传证明 + 运营审核（通过仅入金来源、驳回创建失败）、状态标签（已核验·同名 / 运营审核中 / 已核验·非同名）、列表与出金可用性（仅 verified && sameName）。
...
[2 入金]  打开「收款账户信息」→ 选通道 → 展示收款账号/户主/附言 Reference/时效
          ...
          → 平台自动判定同名: 同名 → 自动入账; 非同名 → 挂账并提示添加该外部账户并上传证明(7 天未添加自动退款 / 可随时手动撤销)
[3 EA]    添加外部账户(同名分流): 选地区(欧洲IBAN / 英国) → 填银行信息
          (持有人可输入; 系统实时判定同名, 大小写/空格不敏感)
          → 同名 → 自动创建成功(免人工, 状态"已核验 · 同名")
          → 非同名 → 上传证明 → 运营审核(同意 → "已核验 · 非同名·仅入金来源"; 驳回 → 创建失败移除)
...
- **I2 入金同名自动 / 非同名挂账闭环**：收款账户户主（holder）= 主体实名；平台引导用户"仅从本人（同名）银行账户转入"；OpenPayd 到账通知含 senderName，平台自动比对：**同名 → 自动入账**；**非同名 → 挂账 pending（不计可用余额），提示用户添加该外部账户并上传证明**；7 天未完成添加 → 平台自动退款；用户可随时手动撤销（立即退款）（决策 D8）。
- **I3 出金只到同名已核验 EA**：可出金目标 = 与主体**同名**且**已核验（verified）**的外部账户；**非同名 EA（即使运营审核通过）仅作入金来源，不可用于出金**；EA 持有人字段（bankAccountHolderName）由用户输入，平台判定同名（大小写 / 空格不敏感），判定结果驱动添加分流（D7 / D9）。
...
- **I5 平台余额 = 平台账本**：用户余额与流水以**平台账本**为准——入金到账即记账；OpenPayd 收款侧资金到账后**自动归集**（收款账户 → 平台结算账户）属平台内部动作，**归集记录不对用户展示**（决策 D6，2026-09-08）；可用余额与在途需清晰区分；交易状态以 OpenPayd 返回为准，平台做展示映射，不虚构中间态。
...
| M2-3 | 到账时效 / 截止 / 限额 | 展示所选通道的 arrive（预计到账）、cutoff（当日截止）、limit（单笔限额）。数值为平台维护的通道元数据（非 OpenPayd 账户字段）：SEPA 2–3 小时（工作日）；SEPA Instant 实时（€100,000）；SWIFT 2–5 个工作日；FPS 实时（£1,000,000）；CHAPS 1–2 小时（工作日） | 原型 03：RAILS 常量（zh/en）｜费率由平台后台配置、C 端只读展示（见决策 D5），时效为平台维护元数据（❓ 与 OpenPayd 对账） |
...
| M3-3 | **同名入金 = 自动入账（平台规则）** | 平台比对到账通知中的 senderName（打款人户名）与收款账户户主（= 主体实名，个人全名 / 企业注册名）是否**同名**（大小写不敏感、忽略空格，F2）：一致 → **系统自动判定同名、直接入账**（免人工；平台余额 +N，流水 payin completed）；**不一致（非同名 / 第三方代付 / 错付）→ 不默认入账，进入非同名挂账闭环（M3-5）**。OpenPayd 本身不校验 sender 与户主同名，此规则为平台强制；同名判定在**入金到账**与**添加外部账户**两侧统一执行（2026-09-08，决策 D4 / D7 / D8） | 用户指定业务规则（2026-09-05 / 2026-09-08）；原型：`amDepLead` + 入金挂账提示条 + 交易记录口径；判定口径（大小写 / 空格）已定，企业后缀等 ❓ F2 |
| M3-4 | 入账口径（平台账本） | 入账增加平台余额（可用）= **平台账本口径**；流水类型 payin、状态 completed、fee 为 0（入金无平台手续费，费率由后台配置，见 D5）；OpenPayd 收款侧资金**自动归集**属内部动作、不进入用户流水（见 D6 / M6-4） | 原型 03：TX payin 示例（fee:0）；管理端 fiat/03-Fiat-Treasury-Overview |
| M3-5 | **非同名入金挂账闭环（核心承诺）** | 非同名 / 无法自动判定同名的入金：① **挂账 pending（不入可用余额）**，顶部琥珀提示条引导两动作：**去添加该外部账户（上传证明）** / **手动撤销（立即退款）**；② 客户为该笔挂账补证（可同时将该外部账户提交非同名 EA 添加审核，见 M4-8）→ 挂账单进入管理端**入金复核队列**（已补证单自动退款停表，等待运营复核）：**批准 → 解除挂账入账**（若同步申请了 EA，通过后标记"已核验 · 非同名·仅入金来源"）；**拒绝 → 原路退款**；③ **7 天内未完成添加与补证 → 平台自动退款**；用户可**随时手动撤销**（立即退款）。⚠️ 退款在 OpenPayd 侧的**执行机制尚未确认**（spec 无退款端点），候选见 §12 F1，需研发确认后定稿 | ❓ F1（退款落地机制）：谁触发、走 OpenPayd 哪个能力（候选：A 反向 bank-payout；B OpenPayd 运营人工原路退回）；挂账 → 引导添加 + 证明 → 审核放行 / 驳回退回、7 天自动退款、手动撤销为**平台规则已定**（2026-09-08，决策 D8） |
| M3-6 | 非同名资金不被误用 | 挂账资金不计入可用余额、不可用于出金 / 兑换；流水状态清晰可解释（"挂账待处理" / 退款后"已退款" / 7 天自动退款"已自动退款"）+ 顶部提示条（含去添加入口 / 手动撤销按钮） | 原型 03：`depPendingNote` 挂账提示条 + 交易记录状态词 + 手动撤销（`.dep-cancel`）；自动退款演示 `dAutoRefund` |

### M4 外部账户（EA）创建 = 新建 Beneficiaries + bank Beneficiaries（同名分流：同名自动成功 / 非同名证明 + 运营审核）

> EA 是用户的**外部银行账户**（出金收款目的地 / 非同名入金的来源登记）。**产品语义（2026-09-08 更新）**：持有人字段**由用户输入**，平台实时判定是否与主体**同名**（大小写 / 空格不敏感）：**同名 → 自动创建成功**（verified，可直接出金）；**非同名 → 须上传证明文件并提交运营审核**（通过 → 标记"非同名 · 仅入金来源"，不可出金；驳回 → EA 移除 = 创建失败）。**OpenPayd 语义**：EA 落库 = 两层对象——① Parent Beneficiary（收款人档案）；② 其下的 Bank Beneficiary（银行收款账户，承载收款账号信息与出金目的地 id）。**OpenPayd 不校验"同名"**（tag 仅为声明式字段），同名判定与审核分流由平台在产品层执行（决策 D7）。

| ID | 需求 | 验收要点 | 对齐 |
| --- | --- | --- | --- |
| M4-1 | 添加向导 = 两步 | ① 选银行所在地区类型：欧洲（EUR，IBAN，SEPA · SWIFT）/ 英国（GBP，Sort Code + 账号，Faster Payments · CHAPS）；② 填写账户信息（持有人可输入 + 实时同名判定，见 M4-2） | 原型 03：eaModal step1/2（eaGrid，EA_TYPES：iban / gb）；向导文案"添加您本人（同名）名下的银行账户，用于同名入金与出金收款" |
| M4-2 | **持有人可输入 + 实时同名判定（分流落点）** | 第 2 步"账户持有人"字段**由用户输入**（可填非本人户名），输入时系统实时比对主体实名（个人：实名全名；企业：注册公司名）自动判定是否**同名**：**大小写不敏感、忽略空格**（xiaoming li / XIAOMING LI / xiao ming li 均判同名）；判定结果驱动后续流程——**同名 → 无需证明，直接提交即自动创建成功**；**非同名 → 须选择证明文件 + 地址信息完整方可提交**（未选证明则提交 disabled），提交后进入运营审核 | 原型 03：holder 输入 + 实时判定提示（同名绿色"提交后将自动创建成功" / 非同名琥珀"需运营审核 + 上传证明"）、`eaf.sameName`、证明必选校验；同名判定口径 ❓ F2（大小写 / 空格已定） |
...
| M4-6 | **同名 EA = 自动创建成功（免人工）** | 同名判定为**同名** → 直接创建成功（verified，状态"已核验 · 同名"），**无需上传证明、无需人工审核**，立即可用于出金；生产可选叠加英国 CoP `POST /beneficiaries/verify` 作为账户真实性增强核验（❓ F4）；欧洲 IBAN 无 CoP 端点（❓ F3） | 原型 03：同名提交 → toast + 状态"已核验 · 同名"（`verified && sameName`）；spec：POST /beneficiaries/verify；同名判定服务（大小写 / 空格归一） |
| M4-7 | EA 列表与出金可用性 | EA 卡片展示：地区旗标 / 名称 / 持有人 / 银行名 / 币种尾号 / 状态标签（**已核验 · 同名** green / **运营审核中** amber / **已核验 · 非同名·仅入金来源** gray）；**出金候选 = 同名且已核验（verified && sameName）**，运营审核中与非同名一律不出现 | 原型 03：renderEa / renderWdList（filter verified && sameName）+ `.chip.gray`（非同名仅入金来源）/ `.chip.amber`（运营审核中）；决策 D9 |
| M4-8 | **非同名运营审核（review → 通过 / 驳回）** | 非同名 EA 提交（含证明）后进入**运营审核中（review）**：**通过 → "已核验 · 非同名（仅入金来源）"**（可作入金来源登记，**不可用于出金**）；**驳回 → EA 移除 = 账户创建失败**（用户可见原因、可重新添加）；审核期间 EA 不可用于出金（置灰 / 不出现于候选）。原型演示：`dRvPass` / `dRvReject` | 管理端 fiat/01-Fiat-Review-Queue（运营审核操作台）；审核记录留存（§9 合规留存） |
...
| M5-2 | 第 1 步：选择收款 EA | 候选 EA = **币种与法币账户一致 + 主体一致（个人账户只列个人 EA，企业账户只列企业 EA）+ 同名（sameName）+ 已核验（verified）**——**非同名 EA（即使运营审核通过）不出现在候选**（严守"出金只能选择正常状态的外部同名账户"）；空态提示先添加（"暂无外部账户，请先添加您的同名银行账户" / 企业对应文案）；可就地从出金跳转添加 EA，添加后返回出金流程 | 原型 03：renderWdList（filter currency + eaSubj==accSubj + **verified && sameName**）；`wdNoEa` / `wdNoEaBiz`；wdAddBtn / fromWithdraw；决策 D9 |
...
| M5-4 | 手续费与到账 | 预估手续费由**后端费率配置**驱动（费率在平台后台配置、C 端只读展示，见决策 D5）：当前默认档 = max(固定费, 金额 × 0.5%)，EUR / GBP 固定费 1.0（❓ 与 OpenPayd 实际费用结构核对后以最终配置为准）；预计到账时效按出金通道（欧洲 IBAN → SEPA；英国 → Faster Payments） | 原型 03：wdFeeFor / RAILS[rail].arrive；管理端 fiat/05-Fiat-Currency-Settings（费率配置，含金额分档）；❓ F5 出金 rails 本期限定 SEPA / FPS 之外是否开放 CHAPS / SWIFT |
...
| M6-4 | **可见性边界：用户只见平台账本** | 用户侧交易记录 / 余额只呈现**平台账本**口径（入金 payin / 提现 payout / 兑换 exchange 等用户资金动作）；**OpenPayd 自动归集、收款账户间资金划拨等为平台内部动作，归集记录不对用户展示**（管理端「资金总览」= 平台账本视图，归集明细仅平台内部可见） | 用户指定业务规则（2026-09-08）；决策 D6；管理端 fiat/03-Fiat-Treasury-Overview |
...
| `dEa` | 打开「添加外部账户」向导（同名分流：同名自动成功 / 非同名证明 + 运营审核） | M4（2026-09-08） |
...
| `dNonSameDep` | 模拟非同名入金到账（300 EUR）→ 挂账待处理 + 提示条 | M3-5（2026-09-08） |
| `dRvPass` | 运营审核通过非同名 EA（review → 已核验 · 非同名·仅入金来源） | M4-8（2026-09-08） |
| `dRvReject` | 运营驳回非同名 EA（review → 移除 = 创建失败） | M4-8（2026-09-08） |
| `dResolveDep` | 运营放行挂账入金（onhold → 入账 completed） | M3-5（2026-09-08） |
| `dAutoRefund` | 模拟 7 天到期自动退款（onhold → 已自动退款） | M3-5（2026-09-08） |
...
**外部账户 EA（平台业务态，2026-09-08 同名分流）**
- **同名**：提交 → 平台同名判定通过 → **自动创建成功（已核验 · 同名）**，立即可出金。
- **非同名**：提交（须已上传证明）→ **运营审核中（review）** → 通过 → **已核验 · 非同名（仅入金来源，不可出金）**；驳回 → EA 移除（创建失败）。
- verified / review 是**平台业务状态**（OpenPayd 的 Bank Beneficiary 对象本身无"审核中 / 已核验"状态字段；对应关系见附录 B）；**出金仅认 `verified && sameName`**（D9）。
- 生产可选增强：英国同名 EA 叠加 CoP `POST /beneficiaries/verify` → status=MATCH（spec 示例值）（❓ F4）；欧洲 IBAN 无 CoP（❓ F3）。
**交易（流水）**
- payin（入金）：`到账通知 → 同名判定：同名 → 自动入账（completed）`；`非同名 → 挂账待处理（pending，提示添加该外部账户并上传证明）→ 运营审核放行 → 入账 / 驳回退回；7 天未添加 → 自动退款；可随时手动撤销 → 已退款（refunded / autoRefunded）`（2026-09-08，决策 D8）。
...
| **非同名入金（第三方代付 / 错付）** | 不默认入账：**挂账 pending（不计可用余额）** + 提示条引导"去添加该外部账户并上传证明"或"手动撤销"；用户添加 + 证明 → 运营审核：放行入账 / 驳回退回；**7 天未添加 → 平台自动退款**；用户可**随时手动撤销**（立即退款）；退款执行机制 OpenPayd 未确认（❓ F1） |
| 入金未填 / 填错 Reference | 归属匹配异常 → 延迟入账 / 人工匹配（M3-2，与 M2-4 一致） |
| 入金户名近似（大小写 / 空格） | 同名判定口径：**大小写不敏感 + 忽略空格**（自动识别大小写，已定 F2）；企业后缀 / 无法自动判定 → 走人工（挂账审核） |
| 账户 PENDING 尝试收款 | 无 rails 收款字段、无操作按钮（M1-3） |
| EA 运营审核中 / 非同名（仅入金来源）选作出金目标 | 均不出现在出金候选（verified && sameName 过滤，M5-2）；运营审核中置灰提示"审核中 · 通过前不可用" |
...
## 10. 埋点 / 可观测（建议）
开户 PENDING→ACTIVE 时长；入金到账 → 上账时长与失败归因；同名判定结果分布与失败率（入金 senderName / EA 持有人两侧）；非同名入金笔数 / 金额 / 处置结果（审核放行 / 7 天自动退款 / 手动撤销）；非同名 EA 运营审核时长与通过率；出金成功率 / 失败原因分布；退款需求笔数（❓ F1 落地后）。
...
生产实现替换关系：`dActivate` → `GET /accounts/{id}` 轮询 / OpenPayd 通知；出金 processing→completed 自动推进 → OpenPayd payout 状态 / webhook；同名 EA 添加自动成功 → 平台同名判定服务（大小写 / 空格归一，英国可叠加 CoP verify）；非同名 EA 审核（`dRvPass` / `dRvReject`）→ 管理端「交易审核」真实运营审核流；入金挂账 / 7 天自动退款 / 手动撤销（`depPendingNote` / `dAutoRefund` / `.dep-cancel`）→ 平台挂账定时任务 + 退款执行（❓ F1 落地机制）。
...
| # | 问题 | 现状 / 张力 | 建议评审结论方向 |
| --- | --- | --- | --- |
| F1 | **退款落地执行机制（原"非同名入金处置"）** | **平台规则已定**（D8 / M3-5）：非同名 → 挂账 → 引导添加账户 + 上传证明 → 运营审核（放行入账 / 驳回退回）；7 天未添加自动退款、可手动撤销。**未定的是退款在 OpenPayd 侧的执行机制**：spec 无 Payout 退款 / 冲正端点（`direct-debit/reverse` 仅针对 direct debit，与出金无关），"谁来退、怎么退"待确认 | 候选：A 平台发起**反向 bank-payout**（原路退回）；B 提交 OpenPayd 运营**人工原路退回**。建议：研发与 OpenPayd 商务确认后定稿；落地前由平台标记退款意图 + 人工执行 |
| F2 | 同名判定口径（入金 senderName 比对 / EA 持有人输入 两侧统一） | **部分已定（2026-09-08）**：大小写不敏感 + 忽略空格（自动识别大小写，xiaoming li / XIAOMING LI 等价）；未定：企业后缀（Ltd / Inc）/ 拼音变体 / 多语言写法 | 企业后缀差异与无法自动判定 → 一律走人工（非同名分流：挂账 / 运营审核）；企业规则由产品 + 合规确认后定稿 |
| F3 | 欧洲 EA 持有人真实性核验手段 | CoP（`POST /beneficiaries/verify`）仅支持英国 sortCode + accountNumber；欧洲 IBAN 账户无对等端点，同名判定由平台执行（大小写 / 空格归一，F2 / M4-2） | 候选：A 平台同名判定（当前默认）+ 大额 / 风控人工复核；B 微额验证（平台能力外，需银行通道支持）；C 引用 KYB 已有银行证明。建议：产品 + 合规拍板 |
...
| # | 决策 | 结论 | 影响 |
| --- | --- | --- | --- |
| D1 | 全局主体（2026-09-05） | 页面单一"当前主体"（INDIVIDUAL / BUSINESS）由演示控制 `dSubject` 决定，**全局驱动**所有弹窗默认字段（开户字段组 / EA 持有人默认名（= 主体名，同名判定基准）/ 出金可用 EA / 入金户主）；移除弹窗内个人 / 企业选择项 | 原型 03 已重构（M0-1）；本 PRD 按此口径书写 |
| D2 | 同名 = 平台强制规则（2026-09-05；2026-09-08 演进为同名分流，见 D4/D7/D8/D9） | OpenPayd 不要求同名（仅 tag=SELF 声明）；YASBe 产品层强制：入金来源须同名（到账自动判定），出金只到**同名且已核验** EA；同名判定升级为**分流**：同名自动通过（添加自动成功 / 入金自动入账）、非同名走证明 + 运营审核 / 挂账退款 | M2-5 / M3-3 / M3-5 / M4-2 / M4-6 / M5-2；§12 F1–F3 |
| D3 | renderTx 容错（2026-09-05） | 历史流水引用的账户被删除时，流水行显示原账户标识兜底而非崩溃 | M6-1 |
| D4 | 出入金审核分流：同名自动 / 非同名人工（2026-09-08） | 出入金统一规则：收款 / 付款对象与主体**同名 → 系统自动审核通过（免人工）**；**非同名 → 证明 + 运营审核 / 挂账**（入金：挂账闭环、引导添加账户与上传证明、7 天自动退款 / 可手动撤销——细化见 D8；出金：非同名一律不可选——见 D9；管理端「交易审核」队列承接运营操作） | M3-3 / M3-5 / M5-2 / M5-7 / §12 F14；管理端 fiat/01-Fiat-Review-Queue；管理端法币域 PRD 将承接平台侧详述 |
| D5 | 费率后台可配置（2026-09-08） | 入金 / 出金费率（固定费 + 比例费，可按金额分档，如 ≤50k / >50k）由**平台后端配置**（管理端「币种设置」），C 端只读展示配置结果、前端不写死费率 | M2-3 / M3-4 / M5-4；管理端 fiat/05-Fiat-Currency-Settings |
| D6 | 归集与平台账本口径（2026-09-08） | 用户入金后 OpenPayd 侧**自动归集**资金（收款账户 → 平台结算账户）；平台只维护**平台账本**（用户余额与流水），归集记录 / 账户间资金划拨为平台内部记录、**不对用户展示** | M3-4 / M6-4；管理端 fiat/03-Fiat-Treasury-Overview（平台账本视图） |
| D7 | 添加外部账户同名分流（2026-09-08） | EA 添加：持有人**用户可输入**，系统实时判定是否同名（**大小写不敏感 + 忽略空格**）：**同名 → 自动创建成功**（免证明免人工，"已核验 · 同名"）；**非同名 → 必须上传证明 + 地址信息完整 → 运营审核（review）**：通过 → "已核验 · 非同名（仅入金来源）"；驳回 → EA 移除（创建失败）。OpenPayd 侧无同名校验，全部由平台执行 | M4-2 / M4-6 / M4-7 / M4-8 / M5-2；原型 03 EA 添加分流（`dRvPass` / `dRvReject`）；同名判定大小写 / 空格归一（F2） |
| D8 | 非同名入金挂账闭环（2026-09-08） | 入金到账自动判定同名：**同名 → 自动入账**；**非同名 → 挂账 pending（不计可用余额）+ 提示添加该外部账户并上传证明**；**7 天未添加 → 平台自动退款**；用户可**随时手动撤销**（立即退款）；用户添加非同名 EA + 证明并经运营审核通过 → 挂账放行入账（该 EA 仅作入金来源，不可出金）。退款在 OpenPayd 侧的执行机制 ❓ F1 | M3-3 / M3-5 / M3-6 / §7 / §8；原型 03：`depPendingNote` + `dAutoRefund` + 手动撤销（`.dep-cancel`）+ `dResolveDep` |
| D9 | 出金仅限同名已核验 EA（2026-09-08） | 出金候选 = **verified && sameName**（币种 + 主体一致前提下）：**非同名 EA 即使运营审核通过（已核验）也不可用于出金**，仅作入金来源——严守"出金只能选择正常状态的外部同名账户" | M5-2 / M4-7 / §7；原型 03 renderWdList 过滤（final 2026-09-08） |
...
| `bankAccountHolderName` | Company Name | **收款账户户主展示名（= 主体实名；入金同名判定比对基准：senderName vs 户主，f_holder）** |
...
| `tag` | — | SELF（本人）/ THIRD_PARTY（他人 / 机构） | 同名 → SELF；非同名（经审核，仅入金来源）→ THIRD_PARTY（❓ 与 OpenPayd 建档约束对齐，D7） |
...
| `bankAccountHolderName` | ✅ | = 用户输入的账户持有人（同名判定落点：同名 = 主体实名自动带入；非同名 = 输入户名，M4-2） |
...
| 外部账户 EA（同名 / 非同名分流，id） | Beneficiary（parent）+ Bank Beneficiary 两层 |
| EA 的 `bankBeneficiaryId` | Bank Beneficiary `id`（出金 `beneficiaryId`） |
| EA 状态（同名已核验 / 运营审核中 / 已核验 · 非同名·仅入金来源） | 同名 → 平台同名判定通过即 verified；非同名 → 运营审核通过（仅入金来源，D7/D8）；OpenPayd 对象无对应状态字段 |
