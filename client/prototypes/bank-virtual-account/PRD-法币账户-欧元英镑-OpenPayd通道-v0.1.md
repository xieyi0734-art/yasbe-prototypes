...
| M3-5 | **非同名入金挂账闭环（核心承诺）** | 非同名 / 无法自动判定同名的入金：① **挂账 pending（不入可用余额）**，顶部琥珀提示条引导两动作：**去添加该外部账户（上传证明）** / **手动撤销（立即退款）**；② 用户添加此外部账户 + 上传证明 → 运营审核（非同名 EA 审核见 M4-8）：**通过 → 挂账资金放行入账**（该 EA 标记"已核验 · 非同名·仅入金来源"）；**驳回 → EA 移除（创建失败）+ 挂账资金退回**；③ **7 天内未添加 → 平台自动退款**；用户可**随时手动撤销**（立即退款）。⚠️ 退款在 OpenPayd 侧的**执行机制尚未确认**（spec 无退款端点），候选见 §12 F1，需研发确认后定稿 | ❓ F1（退款落地机制）：谁触发、走 OpenPayd 哪个能力（候选：A 反向 bank-payout；B OpenPayd 运营人工原路退回）；挂账 → 引导添加 + 证明 → 审核放行 / 驳回退回、7 天自动退款、手动撤销为**平台规则已定**（2026-09-08，决策 D8） |
